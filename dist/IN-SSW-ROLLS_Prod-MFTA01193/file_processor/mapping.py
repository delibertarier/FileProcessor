...
def load_mapping_from_xls(path: Path, sheet_name: Optional[str] = None) -> Mapping:
    """
    Load mapping definition from an XLS file.

    Convention (selected sheet, from row 2 onward):
    - Column 1: source column index (0- or 1-based; non-numeric values skip the row for comments).
    ...
    if sheet_name:
        if sheet_name not in wb.sheetnames:
            raise ValueError(
                f"Mapping sheet {sheet_name!r} not found in {path}. Available sheets: {wb.sheetnames}"
            )
        ws = wb[sheet_name]
    else:
        ws = wb.worksheets[0]
    ...
    for row in raw_rows:
        ...
        try:
            ...
            # Comment / header rows (e.g. "validatie DAS") — ignore entire line.
            continue
